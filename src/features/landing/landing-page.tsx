...
export function LandingPage() {
  return (
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-20 bg-white/95 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/75">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          ...
          <nav className="flex items-center gap-3 text-sm">
            ...
            <Link to="/table" preload="intent" className="rounded-full border border-gray-300 px-3 py-1.5 hover:bg-gray-50">
              Tableau
            </Link>
          </nav>
        </div>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden" style={{ backgroundColor: '#f5c318' }}>
        <div className="mx-auto max-w-6xl px-4 py-16 md:py-20">
          <h1 className="text-center text-3xl font-extrabold text-gray-900 md:text-5xl">
            Trouve tout près de chez toi
          </h1>

          {/* Search */}
          <form action="#" className="mx-auto mt-8 flex max-w-3xl items-stretch gap-0 overflow-hidden rounded-full bg-white p-2 shadow-lg">
            <input
            ...
            <button type="submit" className="ml-2 inline-flex h-12 w-12 items-center justify-center rounded-full" style={{ backgroundColor: '#111827' }} aria-label="Rechercher">
              <IconSearch />
            </button>
          </form>

          {/* Categories */}
          <h2 className="mt-12 text-center text-xl font-bold text-gray-800">Catégories populaires</h2>
          ...
          <div className="mt-6 text-center">
            <button className="rounded-full border border-gray-300 px-4 py-2 text-sm font-medium hover:bg-white/70" style={{ backgroundColor: '#fff' }}>
              Voir toutes
            </button>
          </div>
        </div>

        {/* Side illustrations (placeholder images already in assets) */}
        <img src="/assets/image_004.png" alt="Illustration" className="pointer-events-none absolute -left-10 bottom-0 hidden h-80 select-none md:block" />
        <img src="/assets/image_013.png" alt="Illustration" className="pointer-events-none absolute -right-10 bottom-0 hidden h-80 select-none md:block" />
      </section>

      {/* Footer */}
      <footer className="border-t bg-white">
        <div className="mx-auto max-w-6xl px-4 py-8 text-sm text-gray-500">
          © {new Date().getFullYear()} JBUJB — Tous droits réservés.
        </div>
      </footer>
    </div>
  )
